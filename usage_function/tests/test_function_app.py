...
class TestMonthlyUsage(TestCase):
    """Tests for the monthlyusage/__init__.py file."""
    ...
    def test_main(self) -> None:
        ...
        with patch("monthlyusage.get_all_usage") as mock_get_all, patch(
            "monthlyusage.retrieve_usage"
        ) as mock_retrieve, patch("monthlyusage.get_dates") as mock_get_dates, patch(
            "monthlyusage.send_usage"
        ) as mock_send, patch(
            "utils.settings.get_settings"
        ) as mock_get_settings:
            mock_get_dates.return_value = date(2024, 1, 1), date(2024, 1, 2)
            mock_get_settings.return_value = settings
            monthlyusage.main(mock_timer)
    ...
    def test_get_date_range(self) -> None:
        ...
        with patch("monthlyusage.datetime") as mock_datetime:
            ...
            actual_dates = monthlyusage.get_dates()
